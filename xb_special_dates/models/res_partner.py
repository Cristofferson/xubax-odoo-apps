...
class ResPartner(models.Model):
    ...
    def get_today_pos_reminders(self):
        ...
        _logger.info(
            "[xb_special_dates] get_today_pos_reminders: today=%s scanning %d reminders",
        ...
        for reminder in all_reminders:
            ...
            try:
                ...
                _logger.warning(
                    "[xb_special_dates] _next_occurrence failed for %s: %s",
                    reminder.id, exc
                )
                continue
        ...
        _logger.info(
            "[xb_special_dates] get_today_pos_reminders: matched %d partners",
            len(result)
        )
        ICP = self.env["ir.config_parameter"].sudo()
        try:
            interval = int(
                ICP.get_param("xb_special_dates.pos_popup_interval_hours", "3")
            )
